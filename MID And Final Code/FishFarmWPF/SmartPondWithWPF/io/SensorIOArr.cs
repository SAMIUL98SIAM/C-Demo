    ...
    class SensorIOArr : ParentSensorIOAbstract//ParentSensorIO
    {
        ...
        public override void beginOperation()
        {
            ...
            for (int i = 0; i < TOTAL_TEMP_SENSORS; i++)
            {
                ...
                {
                    Console.WriteLine("Temp data-> id:" + sensor_data_arr[i].sensor_id + "-" 
                        + sensor_data_arr[i].sensor_type
                        + " Date & time=" + sensor_data_arr[i].date_time +
                        " Temp=" + sensor_data_arr[i].data_value);
